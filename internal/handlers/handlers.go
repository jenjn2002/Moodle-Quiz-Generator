...
func BulkDeleteQuestions(w http.ResponseWriter, r *http.Request) {
	user := auth.UserFromCtx(r.Context())
	var req struct {
		IDs []string `json:"ids"`
	}
	...
	if len(req.IDs) == 0 {
		jsonError(w, 400, "no question IDs provided")
		return
	}
	deleted, err := db.DeleteQuestionsBulk(req.IDs, user.ID)
	if err != nil {
		jsonError(w, 500, err.Error())
		return
	}
	jsonResponse(w, 200, map[string]interface{}{"deleted": deleted})
}
